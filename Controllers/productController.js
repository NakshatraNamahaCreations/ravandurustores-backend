...
const productController = {
  ...
  getAllProducts: async (req, res) => {
    try {
      ...
      // 🔥 Normalize images for all products before sending
      products.forEach((p) => {
        p.images = (p.images || []).map(normalizeImagePath).filter(Boolean);
      });
      ...
      if (req.files && req.files.images) {
        ...
        indicesToReplace.forEach((index, i) => {
          if (i < req.files.images.length && newImages[index]) {
            ...
            // ✅ NEW: do NOT delete old file, just change DB reference.
            // This way, any old orders that still point to the old image path
            // will continue to work because the file stays in /uploads/.
            newImages[index] = `/uploads/${req.files.images[i].filename}`;
          }
        });
  ...
  deleteProduct: async (req, res) => {
    try {
      ...
      // ❌ OLD: deleting the files breaks old orders that still point to them
      // allImages.forEach((image) => {
      //   const relativePath = image.startsWith("/") ? image.slice(1) : image;
      //   const imagePath = path.join(__dirname, "..", relativePath);
      //   if (fs.existsSync(imagePath)) {
      ...
      // ✅ NEW: keep image files on disk so that old orders can still display them.
      // If you want to clean these later, do it with a separate maintenance script
      // that checks whether any Order still references the image path.
