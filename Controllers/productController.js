...
const productController = {
  createProduct: async (req, res) => {
    try {
      ...
      const { name, category, stock, description, ingredientsDescription, variants, discountPercentage } = req.body;

      if (!name || !category || stock === undefined || !description || !ingredientsDescription || !variants) {
        return res.status(400).json({ error: "All fields are required." });
      }
      ...
      const product = new Product({
        ...
        variants: parsedVariants,
        images: imagePaths,
        discountPercentage: discountPercentage || 0, // Changed from discountPrice
        soldStock: 0,
      });
  ...
  updateProduct: async (req, res) => {
    try {
      const { name, category, stock, description, ingredientsDescription, variants, discountPercentage, replacedImageIndices, newImageCount } = req.body;
      const product = await Product.findById(req.params.id);
      ...
      if (stock !== undefined) product.stock = stock;
      if (description) product.description = description;
      if (ingredientsDescription) product.ingredientsDescription = ingredientsDescription; // Added new field
      if (discountPercentage !== undefined) product.discountPercentage = discountPercentage; // Changed from discountPrice
