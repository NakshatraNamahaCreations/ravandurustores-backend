...
const orderController = {
  ...
  createOrder: async (req, res) => {
    try {
      ...
      for (const item of items) {
        ...
        if (typeof product.stock !== "number" || product.stock < orderQty) {
          return res.status(400).json({
      ...
      if (Math.abs(Number(amount) - calculatedAmount) > 0.01) {
        return res.status(400).json({
          message: "Amount mismatch. Refresh cart.",
          calculatedAmount,
        });
      }

      // -----------------------------
      // 2️⃣ REDUCE STOCK + INCREASE SOLD
      // -----------------------------
      const formattedItems = [];
      ...
      for (const item of items) {
        ...
        // 🔥 ATOMIC UPDATE (THIS WAS MISSING)
        ...
        if (result.modifiedCount === 0) {
          return res.status(500).json({
            message: "Stock update failed. Order aborted.",
          });
        }

        // image fallback
        let productImage = normalizeImagePath(product.images?.[0]) || "/uploads/default.jpg";
  ...
  // -----------------------------
  // UPDATE ORDER STATUS (CANCEL RESTORES STOCK)
  // -----------------------------
  updateOrderStatus: async (req, res) => {
    try {
      const { status } = req.body;
      ...
      if (status === "Ready for Dispatch" && !order.dispatchDate) {
        order.dispatchDate = new Date();
      }
